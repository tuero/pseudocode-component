...
const pseudocodeCompatPlugin: Plugin = {
  name: "pseudocode-compat-patch",
  setup(build) {
    build.onLoad({ filter: /pseudocode[\\/]src[\\/]Renderer\.js$/ }, async (args) => {
      const fs = await import("fs");
      const text = await fs.promises.readFile(args.path, "utf8");

      return {
        contents: text
          .replace("attrVal = style[attrName];", "var attrVal = style[attrName];")
          .replace(
            "'version': parseInt(MathJax.version.split('.')[0]),",
            "'version': parseInt((typeof MathJax.version === 'string' ? MathJax.version : '0').split('.')[0]),",
          ),
        loader: "js",
      };
    });
...
export default defineConfig({
  ...
  esbuildPlugins: [inlineScriptPlugin, pseudocodeCompatPlugin],
});
